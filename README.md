...
8. **灌溉**：花金币建造蓄水池、沿空地铺设水渠连接地块；地块可设**目标水分**（浇到即止，0=不浇水）
   与**保水优先级**（高→低）；每日结算时，同一连通网络内的**多座蓄水池水量打通、统一分水**，
   按优先级与目标水分逐地块调度，品种耗水（抗旱减半等）计入日耗估算并预测可维持天数；
   降雨/暴雨为蓄水池补水，干旱/酷暑加速蒸发；设施可停用（断流）/启用（恢复）/拆除（返还半价），
   停用、拆除后供水网络即时重算；缺水时面板逐地块展示分配结果（需水/实供），干涸断流会在事件记录中预警
...
`player` `plots` `crops` `crop_varieties` `inventory` `buildings` `animals` `weather_events` `weather_log` `production_jobs` `irrigation` `irrigation_alloc` `breeding_trials`

> 新品种（`crop_varieties`，id 从 1000 起）与基础作物（`crops`）共用 `plots.crop_id`；
> 旧存档启动时自动建表、补列（灌溉优先级/目标水分、分配结果表等）并补插「育种棚」建筑，无需手动迁移。
